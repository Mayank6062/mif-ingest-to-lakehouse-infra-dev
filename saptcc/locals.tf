locals {
  ...
  "kafka-to-iceberg-batch-saptcc-multi-2" = {
    ...
    glue_job_arguments = {
      ...
      "--source_kafka_topic"       = "${local.env}.saptcc.multi-2.raw"
  ...
  "kafka-to-iceberg-batch-saptcc-multi-1" = {
    ...
    glue_job_arguments = {
      ...
      "--sink_iceberg_catalog_type"        = "glue"
      "--sink_iceberg_catalog_id"          = local.miw_account_id[local.env]
      "--sink_iceberg_database"            = "minerva_dev_src_corp_saptcc_prd_raw_db"
      "--sink_iceberg_warehouse"           = "s3://minerva-dev-src-corp/current/prd/raw/saptcc/"
      "--sink_iceberg_checkpoint_dir"      = "s3://minerva-${local.env}-glue-checkpoints/checkpoints/unified/"
      "--sink_iceberg_assume_role_arn"     = "arn:aws:iam::438465132548:role/corp_dtd_dev_procintegratedingestionengineer"
      "--sink_iceberg_assume_session_name" = "mif-glue-iceberg"
